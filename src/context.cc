...
bool Context::init()
{
    float vertex_data[] = { // pos.xyz, normal.xyz, texcoord.uv
    ...
    m_vertex_buffer = Buffer::create_with_data(GL_ARRAY_BUFFER, GL_STATIC_DRAW, vertex_data, sizeof(float) * 8 * 6 * 4);
    ...
    // create shader
    ShaderSharedPtr vertex_shader = Shader::create_from_file("./shader/lighting.vs", GL_VERTEX_SHADER);
    ShaderSharedPtr fragment_shader = Shader::create_from_file("./shader/lighting.fs", GL_FRAGMENT_SHADER);
...
void Context::render()
{
    if (ImGui::Begin("my first ImGui window")) {
        ...
        ImGui::Separator();
        ImGui::Checkbox("animation", &animation_state);
    }
    ImGui::End();
    ...
    m_shader_program->set_uniform("light_color", glm::vec3(1.0f, 1.0f, 1.0f));
    m_shader_program->set_uniform("object_color", glm::vec3(1.0f, 1.0f, 1.0f));
    m_shader_program->set_uniform("ambient_strength", 1.0f);
    m_shader_program->set_uniform("transform", projection * view * lightModelTransform);
    m_shader_program->set_uniform("model_transform", lightModelTransform);
    glDrawElements(GL_TRIANGLES, 36, GL_UNSIGNED_INT, 0);

    m_shader_program->use();
    m_shader_program->set_uniform("light_color", light_color);
    m_shader_program->set_uniform("object_color", object_color);
    ...
    for (size_t i = 0; i < cube_position.size(); i++)
    {
        ...
        model = glm::rotate(model, glm::radians((animation_state ? (float)glfwGetTime() : 0.0f) * 120.0f + 20.0f * (float)i), glm::vec3(1.0f, 0.5f, 0.0f));
